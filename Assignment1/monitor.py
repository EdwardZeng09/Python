...
class Monitor:
    ...
    def _average_wait_time(self) -> float:
        """Return the average wait time of passengers
        that have either been picked
    ...
    def _average_total_distance(self) -> float:
        ...
        """
        acc = 0
        for eachdriver in self._activities[DRIVER].values():
            acc += _h_distance(eachdriver)
        number = len(self._activities[DRIVER])
        return acc / number
    ...
    def _average_trip_distance(self) -> float:
        ...
        acc = 0
        for eachdriver in self._activities[DRIVER].values():
            for activities in range(len(eachdriver)):
                if eachdriver[activities].description == PICKUP:
                    location = eachdriver[activities].location
                    destination = eachdriver[activities + 1].location
...
def _h_distance(lst: list[Activity]) -> int:
    """
    Help get the total distance
    """
    i = 0
    acc = 0
    distance = 0
    while i < len(lst) - 1:
        if lst[i].description == REQUEST:
            loc = lst[i].location
            if lst[i + 1].description == REQUEST:
                des = lst[i + 1].location
                distance = manhattan_distance(loc, des)
                acc += distance
                i += 1
            elif lst[i + 1].description == PICKUP:
                des = lst[i + 1].location
                distance = manhattan_distance(loc, des)
                acc += distance
        elif lst[i].description == PICKUP:
            loc = lst[i].location
            des = lst[i + 1].location
            distance = manhattan_distance(loc, des)
            acc += distance
